...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (addToCartButton) {
    addToCartButton.addEventListener("click", function () {
      ...
      console.log("Article ajouté au localStorage :", cartItems);
    ...
    console.log("⚠️ Bouton 'Ajouter au panier' non trouvé !");
      ...
      // ➤ AJOUT ARTICLE SURPRISE (1 SEULE FOIS)
  ...
  if (!manualAdd) {
    ...
    const alreadyHasSurprise = cartItems.some(item => item.name === "🎁 Article Surprise");

    if (!alreadyHasSurprise && Math.random() < 0.5) {
      const surpriseItem = {
        name: "🎁 Article Surprise",
        price: "CHF 99.-",
      ...
      localStorage.setItem("cartItem", JSON.stringify(cartItems));
      console.log("🎁 Article surprise ajouté :", surpriseItem);
    }
  }
...
if (cartContainer) {
  ...
  // ➤ SUPPRESSION D'UN PRODUIT PRÉCIS
  ...
  // ➤ BOUTON SE CONNECTER / VALIDER LE PANIER
